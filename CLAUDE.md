...
**Simple explanation stays, deep one goes to Science.** Operations and plant files keep a short, simple why right next to their facts, so a line reads on its own without a click. Only the deep explanation (mechanism, derivation) lives in the discipline file. Deduping means removing the repeated *depth*, never stripping the plant side down to a bare link.
