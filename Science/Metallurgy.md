...
- Typical full hardening treatment: austenitize ~840–860°C, soak, oil quench; temper ~150–250°C; target ~58–63 HRC for bearings.
- 60+ HRC hardening physically requires a quench (quench media and their trade-offs: see Quenching below).
...
- Why it causes field failure: a bearing's whole job happens at its surface. A soft decarbed skin on an otherwise-hard part fails in rolling fatigue in the field — and passes a lazy hardness test if the skin isn't ground off before testing.
...
- Demanding bearing-steel specs often call for decarb tolerances in the tens-of-microns range.
...
- Why it matters: it's the state annealed EN31 is supplied in — why it can be machined before hardening. In a quenching shop, pearlite instead of martensite means the parts cooled too slowly.
...
- Why spheres mean soft and machinable: with the hard cementite gathered into rounded particles instead of continuous plates, a cutting tool mostly passes through soft ferrite. This is the target structure of spheroidize annealing (the how of spheroidizing is in the iron-carbon diagram notes).
...
## Heat number
- Industry term: a "heat" = one specific melt/cast batch from the steelmaker, with its own certified composition. The **heat number** is the traceability ID tied to that parent melt — conceptually like a batch/lot number, but assigned by the mill, not by the heat treater.
- Why it matters: if a spectral test shows an out-of-spec reading, the heat number is what lets you trace the anomaly back to a specific melt at the customer's mill rather than guessing it's a factory process error.

## Vocabulary
| Term | Meaning | Why it matters |
|---|---|---|
| Austenite | High-temp structure that dissolves carbon | Starting point of every hardening cycle |
| Martensite | Trapped-carbon hard brittle structure from fast quench | The product of full hardening |
| Pearlite | Layered soft structure from slow cooling | The annealed, machinable state; in a quenching shop, the classic "soft parts" failure mode |
| Hardenability | How deep a steel hardens (not how hard) | Why EN31 can be oil-quenched; why section size matters |
