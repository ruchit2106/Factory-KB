...
How a part is shaped before heat treatment. Heat treatment changes only a part's internal structure, so the shaping route it took decides what internal structure arrives, how much strength the finished part is expected to carry, and how much a decarbed skin matters.
...
- **Why drawing generates heat-treatment work.** Cold drawing is self-limiting: each pass work-hardens the wire further until it becomes too hard and brittle to draw again without cracking. To continue, the wire must be **annealed** — heated to soften it and restore ductility — and then it can be drawn again. That anneal is heat-treatment job work, so a wire-annealing shop sits inside the drawing cycle itself: draw → anneal → draw → anneal.
- A bearing's chain, end to end: mill casts and hot-rolls → cold-drawn to size (with anneals in between) → cut and formed into balls/races → hardened → ground. Nothing in that chain is cast or forged.
...
Both routes almost always still need **machining** (for finish/precision) and **heat treatment** (for properties) afterwards — the extent of each depends on how much precision and how much strength the part actually needs. That "afterwards" is where heat-treatment job work sits in the chain.
...
- But melting is not composition-neutral: carbon and silicon **burn off** in the melt through oxidation, and the ratio drifts every cycle. That's why elements are "added on top" — the melt is corrected up to spec before pouring, and (in a well-run foundry) checked spectrally before it's allowed out (see [[Lab-Testing]], Spectral test).
